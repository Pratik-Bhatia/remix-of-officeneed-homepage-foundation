...
const text = `import { createFileRoute } from '@tanstack/react-router'
...
fs.writeFileSync("src/routes/returns-refunds.tsx", text, "utf8");

// Also let's re-read and write cancellation policy just to be safe
let cancelText = fs.readFileSync("src/routes/cancellation-policy.tsx", "utf8");
// if it has null bytes, read as utf16le
if (cancelText.indexOf('\0') !== -1) {
  cancelText = fs.readFileSync("src/routes/cancellation-policy.tsx", "utf16le");
}
fs.writeFileSync("src/routes/cancellation-policy.tsx", cancelText, "utf8");
