...
function classify(node) {
  ...
  for (const rule of RULES) {
    if (rule.match.test(haystack)) return { type: "regex", sub: rule.sub, match: rule.match.source, matchedString: rule.match.exec(haystack)[0] };
  }
...
.then(json => {
  ...
  const targets = nodes.filter(n => /file|folder|sheet protector/i.test(n.title));
  targets.forEach(t => console.log(t.title, "=>", classify(t)));
})
