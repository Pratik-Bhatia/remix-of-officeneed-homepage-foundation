...
// Remove everything from the first async function finish(final: ChatAnswers) { 
// to the start of const answer = async (opt: string) => {
// Then insert the new function.

const parts = text.split(/async function finish\(final: ChatAnswers\) {.*?const answer = async \(opt: string\) => {/s);
...
if (parts.length === 2) {
  ...
  const answer = async (opt: string) => {`;
    fs.writeFileSync(path, parts[0] + newFunc + parts[1]);
    console.log("Replaced finish function cleanly.");
} else {
    console.log("Regex split failed! Found parts:", parts.length);
}
